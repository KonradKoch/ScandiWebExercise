...
import { getCurrency, getCurrencySymbol, pickCategory } from "../redux/actions/actions";
    ...
    render() {
        ...
        <HeaderDiv>
            ...
            {this.state.categories.map((category, i)=> {
                return (
                    <NavLink id="navbar-category-menu" activeClassName="active" key={i} to={`/${category}/`} value={category} onClick={(e)=>this.pickCategory(e)}>
                    <HeaderUl onClick={(e)=>this.pickCategory(e)} key={category} value={category}>
                        
                        <HeaderLi value={category} key={category}>{category}</HeaderLi>
        ...
        <DropDownHeader onClick={() => this.togglingCurrencyMenu()}>
            ...
            <DropDownListContainer>
                <DropDownList style={{display: !this.state.isOpen? "none" : ""}}>
                    {this.state.isOpen && this.state.currencies.map((currency, i)=> {
                        const currSymbol = this.state.symbols[i];
                        return (
                            
                            <ListItem key={currSymbol +" "+ currency} onClick={(e) => this.changeCurrency(e)} id={currSymbol} value={currency}> {currSymbol} {currency} </ListItem>
